...
func (o *openAPI) init(webServices []*restful.WebService) error {
	...
	if o.config.GetDefinitionName == nil {
		o.config.GetDefinitionName = func(_, name string) (string, []string) {
			return name[strings.LastIndex(name, "/")+1:], []string{}
		}
	}
	o.definitions = o.config.GetDefinitions(func(name string) spec.Ref {
		defName, _ := o.config.GetDefinitionName(o.servePath, name)
		return spec.MustCreateRef("#/definitions/" + escapeJsonPointer(defName))
	})
...
func (o *openAPI) buildDefinitionRecursively(name string) error {
	// If the type we are looking at is in vendor folder, we should just look
	// at the actual path instead of vendored path.
	...
	if item, ok := o.definitions[name]; ok {
		schema := spec.Schema{
			...
			VendorExtensible: spec.VendorExtensible{
				Extensions: spec.Extensions{ExtensionTags: tags},
			},
		}
...
func (o *openAPI) buildDefinitionForType(sample interface{}) (string, error) {
	...
	defName, _ := o.config.GetDefinitionName(o.servePath, name)
	return "#/definitions/" + escapeJsonPointer(defName), nil
}
