...
func getAllServingCRDVersions(crd *apiextensionsv1beta1.CustomResourceDefinition) []string {
...
func checkResourceExistsInDiscovery(crd *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface, version string) (bool, error) {
...
func CreateNewCustomResourceDefinitionWatchUnsafe(crd *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface) (*apiextensionsv1beta1.CustomResourceDefinition, error) {
	...
	for _, v := range getAllServingCRDVersions(crd) {
		version := v
		err := wait.PollImmediate(500*time.Millisecond, 30*time.Second, func() (bool, error) {
			return checkResourceExistsInDiscovery(crd, apiExtensionsClient, version)
		})
		if err != nil {
			return nil, err
		}
...
func CreateNewCustomResourceDefinition(crd *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface, dynamicClientSet dynamic.Interface) (*apiextensionsv1beta1.CustomResourceDefinition, error) {
	...
	for _, v := range getAllServingCRDVersions(crd) {
		version := v
		err := wait.PollImmediate(500*time.Millisecond, 30*time.Second, func() (bool, error) {
			return checkForWatchCachePrimed(crd, dynamicClientSet, version)
		})
		if err != nil {
			return nil, err
		}
...
// checkForWatchCachePrimed returns true if the watch is primed for an specified version of CRD watch
func checkForWatchCachePrimed(crd *apiextensionsv1beta1.CustomResourceDefinition, dynamicClientSet dynamic.Interface, version string) (bool, error) {
	ns := ""
	...
	case watchEvent := <-noxuWatch.ResultChan():
		if watch.Error == watchEvent.Type {
			return false, fmt.Errorf("watch cache is not primed yet, retrying")
		}
...
func DeleteCustomResourceDefinition(crd *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface) error {
	...
	for _, v := range getAllServingCRDVersions(crd) {
		version := v
		err := wait.PollImmediate(500*time.Millisecond, 30*time.Second, func() (bool, error) {
			exists, err := checkResourceExistsInDiscovery(crd, apiExtensionsClient, version)
			return !exists, err
		})
		if err != nil {
			return err
