...
func (s *APIAggregator) updateOpenAPISpec() error {
	...
	for name, proxyHandler := range s.proxyHandlers {
		proxySpec, priority := proxyHandler.OpenAPISpec()
		if proxySpec == nil {
			continue
		}
		specs = append(specs, openAPISpecInfo{name, proxySpec, priority})
	}
...
func (s *APIAggregator) AddAPIService(apiService *apiregistration.APIService) error {
	...
	if proxyHandler, exists := s.proxyHandlers[apiService.Name]; exists {
		if err := proxyHandler.updateAPIService(apiService); err != nil {
			return err
		}
	...
	proxyHandler := &proxyHandler{
		...
		disableOpenAPI:  s.openAPIService == nil,
	}
	if err := proxyHandler.updateAPIService(apiService); err != nil {
		return err
	}
