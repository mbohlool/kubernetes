...
func newWalkAllRef(walkRef func(ref spec.Ref) spec.Ref, sp *spec.Swagger) *walkAllRefs {
	return &walkAllRefs{
...
func FilterSpecByPaths(sp *spec.Swagger, keepPathPrefixes []string) {
	...
	newWalkAllRef(func(ref spec.Ref) spec.Ref {
		if ref.String() != "" {
			refStr := ref.String()
			if strings.HasPrefix(refStr, DEFINITION_PREFIX) {
...
// Copy paths and definitions from source to target, rename definitions if needed.
func MergeSpecs(target, source *spec.Swagger) error {
	source, err := CloneSpec(source)
	if err != nil {
		return err
	}
	for k, v := range source.Paths.Paths {
		if _, found := target.Paths.Paths[k]; found {
			return fmt.Errorf("Unable to merge: Duplicated path %s", k)
		}
		target.Paths.Paths[k] = v
	}
	...
	for k, v := range source.Definitions {
		...
		if found || usedNames[k] {
			...
			renameDefinition(source, k, newName)
		}
		usedNames[k] = true
	}
	for k, v := range source.Definitions {
		if _, found := target.Definitions[k]; !found {
