...
func Test_PVLAdmission(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			err := handler.Admit(admission.NewAttributesRecord(testcase.preAdmissionPV, nil, api.Kind("PersistentVolume").WithVersion("version"), testcase.preAdmissionPV.Namespace, testcase.preAdmissionPV.Name, api.Resource("persistentvolumes").WithVersion("version"), "", admission.Create, false, nil, nil), nil)
