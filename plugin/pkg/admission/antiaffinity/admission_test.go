...
func TestInterPodAffinityAdmission(t *testing.T) {
	...
	for _, test := range tests {
		...
		err := handler.Validate(admission.NewAttributesRecord(&pod, nil, api.Kind("Pod").WithVersion("version"), "foo", "name", api.Resource("pods").WithVersion("version"), "", "ignored", false, nil, nil), nil)
...
func TestOtherResources(t *testing.T) {
	...
	for _, tc := range tests {
		...
		err := handler.Validate(admission.NewAttributesRecord(tc.object, nil, api.Kind(tc.kind).WithVersion("version"), namespace, name, api.Resource(tc.resource).WithVersion("version"), tc.subresource, admission.Create, false, nil, nil), nil)
