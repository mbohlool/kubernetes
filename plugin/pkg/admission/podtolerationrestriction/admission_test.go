...
func TestIgnoreUpdatingInitializedPod(t *testing.T) {
	...
	err = handler.Admit(admission.NewAttributesRecord(pod, pod, api.Kind("Pod").WithVersion("version"), "testNamespace", pod.ObjectMeta.Name, api.Resource("pods").WithVersion("version"), "", admission.Update, false, nil, nil), nil)
	if err != nil {
